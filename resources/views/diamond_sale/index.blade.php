            ...
            <div class="col-md-12 mb-4">
                <div class="card text-left">
                    <div class="card-header text-right bg-transparent">
                        @can('diamond_sale_create')
                        <a class="btn btn-primary btn-md m-1" href="{{ url('diamond-sale/create') }}"><i
                                class="fa fa-plus text-white mr-2"></i> Add Diamond Sale</a>
                        @endcan
                    </div>
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-8" id="div_post_diamond_sale">
                                ...
                                <a class="btn btn-danger" style="color:#fff;" type="button" id="unselectAll">Uncheck
                                    All</a>
                                @can('diamond_sale_post')
                                <a class="btn btn-primary" style="color:#fff;" type="button" id="post_sale">Post</a>
                                @endcan
                            </div>
                        </div>
                        <div class="row mt-2" id="diamond_sale_account" style="display: none;">
                            <div class="col-md-3">
                                <div class="form-group">
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table id="diamond_sale_table" class="display table" style="width:100%">
                                    <thead>
                                        <tr>
                                            <th></th>
                                            <th>Sr#</th>
...
{data: 'diamond_sale_date', name: 'diamond_sale_date', orderable: false, searchable: false},
{data: 'diamond_sale_no', name: 'diamond_sale_no', orderable: false, searchable: false},
...
'route' => 'diamond-sale/data',
'buttons' => false,
'pageLength' => 50,
'class' => 'diamond_sale_table',
'variable' => 'diamond_sale_table',
'datefilter' => true,
...
<script type="text/javascript">
    $('#search_button').click(function() {
        $("#preloader").show();
        initDataTablediamond_sale_table();
        $("#div_post_diamond_sale").show();
        $("#preloader").hide();
    });
    ...
    $("#post_sale").click(function() {
        ...
        $(".sub_chk:checked").each(function() {
            ...
            diamond_sales.push($(this).val());
        });
        var data = {};
        data.diamond_sale = diamond_sales;
        ...
        if (diamond_sales.length <= 0) {
            $("#preloader").hide();
            error("Please select Rows!");
        } else {
            $.ajax({
                url: "{{ url('diamond-sale/post') }}",
                type: "POST",
                headers: {
    ...
    $("body").on("click", "#unpost_diamond_sale", function() {
        $("#preloader").show();
        var sale_id = $(this).data("id");

        $.ajax({
            url: "{{ url('diamond-sale/unpost') }}/" + sale_id,
            type: "GET",
            headers: {
                "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
            },
    ...
    $("body").on("click", "#deleteOtherSale", function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                        type: "get",
                        url: "{{ url('diamond-sale/destroy') }}/" + diamond_sale_id,
                    }).done(function(data) {
                        if ((data.Success = true)) {
                            $("#preloader").hide();
                            success(data.Message)
    ...
    $("body").on("click", ".sub_chk", function() {
        if ($("input[type=checkbox]").is(':checked')) {
            $("#diamond_sale_account").show();
        } else {
            $("#diamond_sale_account").hide();
        }
    });
</script>
@endsection