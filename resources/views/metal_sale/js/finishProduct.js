...
$("body").on("click", "#purchase_id", function (event) {
    ...
    $.ajax({
        ...
        if (data.Success) {
            var data = data.Data;
            $("#metal_purchase_id").val((data.metal_purchase_id > 0) ? data.metal_purchase_id : 0);
            ...
            BeadByPurchaseDetail(data.metal_purchase_id, product_id);
            StonesByPurchaseDetail(data.metal_purchase_id, product_id);
            DiamondsByPurchaseDetail(data.metal_purchase_id, product_id);
        } else {
            error(data.Message);
        }
    });
...
function TotalAmount() {
    ...
    var making = $("#making").val();
    var laker = $("#laker").val();
    var total_gold_amount = $("#total_gold_amount").val();
    var cal = (total_bead_amount * 1) + (total_stones_amount * 1) + (total_diamond_amount * 1) + (other_amount * 1) + (making * 1) + (laker * 1) + (total_gold_amount * 1);
...
$("body").on("click", "#submit", function (e) {
    ...
    if ($("#other_charges").val() == '') {
        error("Please enter other charges, minimum 0!");
        $("#other_charges").focus();
        return false;
    }
    if ($("#metal_rate").val() == '' || $("#metal_rate").val() == 0) {
        error("Please enter metal rate!");
        $("#metal_rate").focus();
        return false;
    }
    if ($("#total_metal_amount").val() == '' || $("#total_metal_amount").val() == 0) {
        error("Please enter total metal amount!");
        $("#total_metal_amount").focus();
        return false;
    }
