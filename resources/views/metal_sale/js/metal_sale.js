...
function getMetalProduct(tag_no) {
    $("#preloader").show();
    $.ajax({
        url: url_local + "/metal-product/get-by-tag-no/" + tag_no,
        type: "GET",
    }).done(function (data) {
        console.log(data);
        if (data.Success) {
            ...
            if (Array.isArray(data)) {
                ...
                $("#metal_product_id").val(data.id > 0 ? data.id : "");
                $("#metal_purchase_id").val(
                    data.metal_purchase_id > 0 ? data.metal_purchase_id : ""
                );
                $("#metal_purchase_detail_id").val(
                    data.metal_purchase_detail_id > 0 ? data.metal_purchase_detail_id : ""
                );
                ...
                $("#metal_rate").val(data.metal_rate > 0 ? data.metal_rate : 0).trigger("keyup");
                $("#metal").val(data.metal != "" ? data.metal : 0);
                $("#purity").val(data.purity != "" ? data.purity : 0);
                $("#scale_weight").val(
                ...
                $("#other_charges").val(
                    data.other_charges > 0 ? data.other_charges : 0
                );
                BeadByMetalDetail(data.id);
                StonesByMetalDetail(data.id);
                DiamondsByMetalDetail(data.id);
                TotalMetalAmount();
                TotalAmount();
            }
            $("#preloader").hide();
...
$("#metal_rate").on("keyup", function (event) {
    ...
    var total = (gross_weight * 1) * (metal_rate * 1);
    $("#total_metal_amount").val(total.toFixed(3));
    TotalMetalAmount();
    TotalAmount();
});
...
function ChildAdd(data) {
    ...
    $.each(data, function (e, val) {
        productData.push({
            // sr: i,
            tag_no: val.tag_no,
            metal_product_id: val.metal_product_id,
            metal_purchase_id: val.metal_purchase_id,
            metal_purchase_detail_id: val.metal_purchase_detail_id,
            // job_purchase_detail_id: val.job_purchase_detail_id,
            product: val.product,
            product_id: val.product_id,
            metal: val.metal,
            metal_rate: val.metal_rate,
            purity: val.purity,
            ...
            total_metal_amount: val.total_metal_amount,
            other_charges: val.other_charges,
            total_bead_amount: val.total_bead_amount,
            total_stones_amount: val.total_stones_amount,
            total_diamond_amount: val.total_diamond_amount,
    ...
    $.each(productData, function (e, val) {
        ...
        rows += `<tr id="${val.metal_product_id}"><td>${product_sr}</td><td>${val.tag_no}</td><td>${val.product}</td>
          ...
          <td style="text-align: right;" >${val.total_gold_price}</td>
          <td style="text-align: right;" >${val.other_charges}</td><td style="text-align: right;" >${val.total_amount}</td>
          <td><a class="text-warning text-white" id="Detail" href="javascript:void(0)" data-toggle="tooltip"  data-id="${val.metal_product_id}" data-original-title="Detail"><i title="Detail" class="mr-2 fa fa-eye"></i></a>
          <a class="text-danger text-white productr${val.metal_product_id}" onclick="ProductRemove(${val.metal_product_id})"><i class="fa fa-trash"></i></a></td></tr>`;
...
function addProduct() {
    ...
    var metal_product_id = $("#metal_product_id").val();
    var metal_purchase_id = $("#metal_purchase_id").val();
    var metal_purchase_detail_id = $("#metal_purchase_detail_id").val();
    ...
    var check = true;
    $.each(productData, function (e, val) {
        if (val.metal_product_id == $("#metal_product_id").val()) {
    ...
    productData.push({
        // sr: i,
        tag_no: tag_no,
        metal_product_id: metal_product_id,
        metal_purchase_id: metal_purchase_id,
        metal_purchase_detail_id: metal_purchase_detail_id,
    ...
    $.each(productData, function (e, val) {
        ...
        rows += `<tr id="${val.metal_product_id}"><td>${product_sr}</td><td>${val.tag_no}</td><td>${val.product}</td>
                <td>${val.metal}</td><td style="text-align: right;">${val.purity}</td><td style="text-align: right;">${val.scale_weight}</td><td style="text-align: right;">${val.bead_weight}</td>
                <td style="text-align: right;">${val.stones_weight}</td><td style="text-align: right;">${val.diamond_weight}
                <td style="text-align: right;">${val.net_weight}</td>
                </td><td style="text-align: right;">${val.gross_weight}</td><td style="text-align: right;" >${val.metal_rate}</td>
          <td style="text-align: right;" >${val.total_metal_amount}</td>
          <td style="text-align: right;" >${val.other_charges}</td><td style="text-align: right;" >${val.total_amount}</td>
          <td><a class="text-warning text-white" id="Detail" href="javascript:void(0)" data-toggle="tooltip"  data-id="${val.metal_product_id}" data-original-title="Detail"><i title="Detail" class="mr-2 fa fa-eye"></i></a>
          <a class="text-danger text-white productr${val.metal_product_id}" onclick="ProductRemove(${val.metal_product_id})"><i class="fa fa-trash"></i></a></td></tr>`;
...
$("body").on("click", "#Detail", function (event) {
    ...
    let data = productData.filter(
        (product) => product.metal_product_id === String($(this).data("id"))
    );
    var bead_sr = 0;
    var bead_row = "";
    var stone_sr = 0;
...
$("body").on("click", "#submit", function (e) {
    ...
    $("#preloader").show();
    // Validation logic
    if ($("#metal_sale_date").val() == "") {
        error("Please select metal sale date!");
        $("#metal_sale_date").focus();
        $("#preloader").hide();
        return false;
    }
    ...
    formData.append("metal_sale_date", $("#metal_sale_date").val());
    ...
    $.ajax({
        url: url_local + "/metal-sale/store", // Laravel route
        type: "POST",
        headers: {
            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
        },
        ...
        success: function (data) {
            if (data.Success) {
                ...
                setTimeout(function () {
                    ...
                    window.location = url_local + "/metal-sale";
