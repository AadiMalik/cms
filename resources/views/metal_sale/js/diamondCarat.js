...
function addDiamond() {
    ...
    $.each(diamondsData, function (e, val) {
        ...
        rows += `<tr id=${type.replace(/\s+/g, '')+ metal_product_id + Math.floor(val.diamonds) + Math.floor(val.carat)}><td>${i}</td><td>${val.diamonds}</td><td>${val.type}</td>
          ...
          <td><a class="text-danger text-white diamondr${type.replace(/\s+/g, '') + metal_product_id + Math.floor(val.diamonds) + Math.floor(val.carat)}" onclick="DiamondRemove('${type.replace(/\s+/g, '') + metal_product_id + Math.floor(val.diamonds) + Math.floor(val.carat)}')"><i class="fa fa-trash"></i></a></td></tr>`;
...
function DiamondsByMetalDetail(metal_product_id) {
    $.ajax({
        type: "GET",
        url: url_local + "/metal-product/get-diamond-by-id" + "/" + metal_product_id,
        headers: {
