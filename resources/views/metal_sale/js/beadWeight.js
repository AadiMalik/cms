...
function addBead() {
    ...
    // rows += `<tr id=${type.replace(/\s+/g, '') + Math.floor(beads) + Math.floor(bead_gram)}><td>${i}</td><td>${type}</td><td>${beads}</td><td style="text-align: right;">${bead_gram}</td><td style="text-align: right;" >${bead_carat}</td>
    //       <td style="text-align: right;" >${bead_gram_rate}</td><td style="text-align: right;" >${bead_carat_rate}</td><td style="text-align: right;" >${bead_total}</td>
    //       <td><a class="text-danger text-white r${type.replace(/\s+/g, '') + Math.floor(beads) + Math.floor(bead_gram)}" onclick="Remove(${type.replace(/\s+/g, '') + Math.floor(beads) + Math.floor(bead_gram)})"><i class="fa fa-trash"></i></a></td></tr>`;
    // var total = $("#total_bead_amount").val() * 1 + bead_total * 1;
    // var total_weight = $("#bead_weight").val() * 1 + bead_total * 1;
    ...
    $.each(beadData, function (e, val) {
        ...
        beadData.sr = i;
        var type = val.type;
        rows += `<tr id=${type.replace(/\s+/g, '') + metal_product_id + Math.floor(val.beads) + Math.floor(val.gram)}><td>${i}</td><td>${val.type}</td><td>${val.beads}</td><td style="text-align: right;">${val.gram}</td><td style="text-align: right;" >${val.carat}</td>
            ...
            <td><a class="text-danger text-white r${type.replace(/\s+/g, '') + metal_product_id + Math.floor(val.beads) + Math.floor(val.gram)}" onclick="Remove('${type.replace(/\s+/g, '') + metal_product_id + Math.floor(val.beads) + Math.floor(val.gram)}')"><i class="fa fa-trash"></i></a></td></tr>`;
...
function BeadByMetalDetail(metal_product_id) {
    $.ajax({
        type: "GET",
        url: url_local + "/metal-product/get-bead-by-id" + "/" + metal_product_id,
        headers: {
...
function Remove(id) {
    ...
    Swal.fire({
        ...
        $.each(beadData, function (i, val) {
            var type = val.type;
            if (type.replace(/\s+/g, '') + Math.floor(val.beads) + metal_product_id + Math.floor(val.gram) == id) {
