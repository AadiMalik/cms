...
var metalSaleOrderData = [];
var metal_sale_order_sr = 0;
Clear();
...
$("#product_id").on("change", function () {
    ...
    $("#preloader").show();
    $.ajax({
        url: url_local + "/metal-sale-order/get-product-mol/" + product_id,
        type: "GET",
        headers: {
            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
        },
...
function addProduct() {
    ...
    var check = true;
    $.each(metalSaleOrderData, function (e, val) {
        if (
    ...
    $.each(metalSaleOrderData, function (e, val) {
        metal_sale_order_sr = metal_sale_order_sr + 1;
        metalSaleOrderData.sr = metal_sale_order_sr;

        rows += `<tr id="row_${val.product_id}_${val.category}_${val.metal}_${val.design_no}"><td>${metal_sale_order_sr}</td><td>${val.product}</td><td>${val.category}</td>
                <td>${val.design_no}</td><td>${val.metal}</td>
                <td style="text-align: right;">${val.rate}</td><td>${val.purity}</td><td style="text-align: right;">${val.net_weight}</td>
                <td style="text-align: right;">${val.gross_weight}</td>
                <td>${val.description}</td>
                <td><a class="text-danger text-white metalsaleorderr${val.product_id}" onclick="MetalSaleOrderRemove('${val.product_id}','${val.category}','${val.metal}','${val.design_no}')"><i class="fa fa-trash"></i></a></td></tr>`;

    });
    success("Product Added Successfully!");
    $("#metal_sale_order_products").empty();
    console.log(rows);
    $("#metal_sale_order_products").html(rows);
    Clear();
    MetalSaleOrderShort();
    $("#preloader").hide();
}
...
$("body").on("click", "#submit", function (e) {
    ...
    $("#preloader").show();
    // Validation logic
    if ($("#metal_sale_order_date").val() == "") {
        error("Please select sale date!");
        $("#metal_sale_order_date").focus();
        $("#preloader").hide();
        return false;
    }

    if ($("#metal_delivery_date").val() == "") {
        error("Please select delivery date!");
        $("#metal_delivery_date").focus();
        $("#preloader").hide();
        return false;
    }
    ...
    formData.append("metal_sale_order_date", $("#metal_sale_order_date").val());
    ...
    formData.append("metalSaleOrderDetail", JSON.stringify(metalSaleOrderData));

    $.ajax({
        url: url_local + "/metal-sale-order/store", // Laravel route
        type: "POST",
        headers: {
            "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
        },
        ...
        success: function (data) {
            if (data.Success) {
                ...
                setTimeout(function () {
                    ...
                    window.location = url_local + "/metal-sale-order";
...
function MetalSaleOrderRemove(product_id, category, metal, design_no) {
    console.log(id);
    Swal.fire({
        title: "Are you sure?",
        ...
        var total = 0;
        $("#preloader").show();
        $.each(metalSaleOrderData, function (i, val) {
            if (
                val.product_id == product_id &&
                val.category == category &&
                val.design_no == design_no &&
                ...
                $("#row_" + val.product_id + "_" + val.category + "_" + val.metal + "_" + val.design_no).remove();
                item_index = i;
                return false;
            }
        });
        if (item_index !== "") {
            metalSaleOrderData.splice(item_index, 1);
        }
        // var check = ".metalsaleorderr" + id;
...
// Short
function MetalSaleOrderShort() {
    var tbody, j, x, y;
    tbody = document.getElementById("metal_sale_order_products");
    var switching = true;
