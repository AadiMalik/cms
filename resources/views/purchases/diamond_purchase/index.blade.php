            ...
            <div class="col-md-12 mb-4">
                <div class="card text-left">
                    <div class="card-header text-right bg-transparent">
                        @can("diamond_purchase_create")
                        <a class="btn btn-primary btn-md m-1" href="{{ url('diamond-purchase/create') }}"><i
                                class="fa fa-plus text-white mr-2"></i> Add Diamond Purchase</a>
                        @endcan
                    </div>
                    <div class="card-body">
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table id="diamond_purchase_table" class="display table" style="width:100%">
                                    <thead>
                                        <tr>
                                            <th></th>
                                            <th>Sr#</th>
...
{data: 'diamond_purchase_date', name: 'diamond_purchase_date', orderable: false, searchable: false},
{data: 'diamond_purchase_no', name: 'diamond_purchase_no', orderable: false, searchable: false},
...
'buttons' => false,
'pageLength' => 50,
'class' => 'diamond_purchase_table',
'variable' => 'diamond_purchase_table',
'datefilter' => true,
...
<script type="text/javascript">
    $('#search_button').click(function() {
        $("#preloader").show();
        initDataTablediamond_purchase_table();
        $("#preloader").hide();
    });
    ...
    $("#post").click(function() {
        $("#preloader").show();
        var diamond_purchase = [];
        $(".sub_chk:checked").each(function() {

            diamond_purchase.push($(this).val());
        });
        var data = {};
        data.diamond_purchase = diamond_purchase;
        if (diamond_purchase.length <= 0) {
            $("#preloader").hide();
            error("Please select Rows!");
        } else {
            $.ajax({
                url: "{{ url('diamond-purchase/post') }}",
                type: "POST",
                headers: {
    ...
    $("body").on("click", "#unpost", function() {
        ...
        $.ajax({
            url: "{{ url('diamond-purchase/unpost') }}/" + diamond_purchase_id,
            type: "GET",
            headers: {
                "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
            },
    ...
    $("body").on("click", "#deleteDiamondPurchase", function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                        type: "get",
                        url: "{{ url('diamond-purchase/destroy') }}/" + diamond_purchase_id,
                    }).done(function(data) {
                        if ((data.Success = true)) {
                            $("#preloader").hide();
                            success(data.Message)
