...
function addBead() {
    ...
    // var total = $("#total_bead_price").val() * 1 + bead_total * 1;
...
function BeadByPurchaseDetail(metal_purchase_detail_id) {
    $.ajax({
        type: "GET",
        url: url_local + "/metal-purchase/beads" + "/" + metal_purchase_detail_id,
        headers: {
...
function Remove(id) {
    ...
    Swal.fire({
        ...
        $.each(beadData, function (i, val) {
            var type = val.type;
            if (type.replace(/\s+/g, '') + Math.floor(val.beads) + Math.floor(val.gram) == id) {
